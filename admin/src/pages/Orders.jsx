...
const Orders = ({token}) => {
  ...
  const fetchAllOrders = async () => {
    ...
    try {
      const response = await axios.post(backentUrl + '/api/order/list',{},{headers: {token}})
  ...
  const statusHandler = async (event, orderId) => {
    try {
      const response = await axios.post(backentUrl + '/api/order/status', { orderId, status:event.target.value}, {headers:{token}})
      if(response.data.success){
      ...
      <div>
        {
          orders.map((order, index)=>(
              <div key={index} className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_2fr_1fr_1fr_1fr] gap-3 items-start border-2 border-gray-200 p-5 md:p-8 my-3 md:my-4 text-xs sm:text-sm text-gray-700  '> 
                ...
                <p className='mt-3 mb-2 font-mediam'>{order.address.firstName + " " + order.address.lastName}</p>
                <div>
